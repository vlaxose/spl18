...
for path_indx = 1:length(paths_range)
  ...
  for sub_indx=1:length(T_range)
   ...
   disp(['realization: ', num2str(r)]);
    ...
    s_twostage = vamp(vec(X_twostage_1), kron(conj(Dt), Dr), 0.001, 2*Np);
   ...
   mean_error_proposed(sub_indx, path_indx) = mean(error_proposed);
   mean_error_omp(sub_indx, path_indx) = mean(error_omp);
   mean_error_vamp(sub_indx, path_indx) = mean(error_vamp);
   mean_error_twostage(sub_indx, path_indx) = mean(error_twostage);
